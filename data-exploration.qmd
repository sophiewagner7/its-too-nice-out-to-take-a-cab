...
df |> 
  mutate(
    is_weekend = factor(
      ...
      levels = c("Weekend", "Weekday") # Ensures Weekend appears before Weekday
    )
  ) |> 
  ggplot() +
  geom_line(aes(x = hour, y = trip_count, color = is_weekend, group = date), alpha = 0.3) +
  scale_color_manual(
    values = c("Weekend" = "grey", "Weekday" = base_color), 
    name = "Day Type" # Legend title
  ) +
  scale_x_continuous(breaks=seq(0,23,by=3))+
  labs(
    title = "Trip Counts by Hour and Day Type",
    x = "Hour of the Day",
    y = "Trip Count"
  ) +
  theme_minimal()
...
df |> 
  ...
  ggplot(aes(x = hour, y = trip_count, color = is_weekend)) +
  geom_smooth(se=T) +
