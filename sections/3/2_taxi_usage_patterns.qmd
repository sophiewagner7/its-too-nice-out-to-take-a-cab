...
p1 <- df |> 
    mutate(rush_hour_group = "Overall") |> 
    mutate(rush_hour_group = factor(rush_hour_group, 
                                    levels = c("Overall", "Morning rush hour (7-9am)", "Evening rush hour (5-7pm)"), ordered = TRUE)) |> 
  ggplot(aes(x = month_abbr, y = trip_count, fill=rush_hour_group)) +
  ...
  labs(title = "Number of trips by month", x = "Month", y = "Number of trips", fill="") +
  theme_minimal() 
